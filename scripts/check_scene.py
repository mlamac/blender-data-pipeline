...
required = {"Density_Volume", "Domain_Wireframe", "Camera", "Key_Light"}
...
for image in bpy.data.images:
    if image.source in {"FILE", "SEQUENCE"} and image.filepath and not image.filepath.startswith("//assets/slices/"):
...
if bpy.context.scene.frame_end > 1:
    ...
    if any(image.source != "SEQUENCE" for image in bpy.data.images if image.filepath):
